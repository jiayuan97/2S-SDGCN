...
def create_spectral_features(args, positive_edges, negative_edges, node_count):
    ...
    print(max(int(max(index_1)), int(max(index_2))))
    shaping = (max(int(max(index_1)), int(max(index_2)) + 1), max(int(max(index_1)), int(max(index_2))) + 1)
...
def get_bfs_sub_graph(ppi_list, node_num, node_to_edge_index, sub_graph_size):
    ...
    random_node = random.sample(node_to_edge_index.keys(), 1)[0]
    ...
    #     if edge not in selected_edge_index:

    return selected_edge_index, list(set(range(len(ppi_list))) - set(selected_edge_index))


def get_dfs_sub_graph(edges, node_num, node_to_edge_index, sub_graph_size):
    stack = []
    selected_edge_index = []
    selected_node = []
