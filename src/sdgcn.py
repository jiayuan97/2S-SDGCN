...
class SignedGraphConvolutionalNetwork(torch.nn.Module):
    ...
    def setup_layers(self):
        ...
        print(self.positive_aggregators)
    ...
    def calculate_regression_loss(self, z, target):
        ...
        #  if True:
        #      with open('./OTC_target.pkl', 'wb') as f:
        #          pickle.dump(targets, f)
        # # print(predictions_soft.shape)
        #  if True:
        #      with open('./OTC_prediction1.pkl', 'wb') as f:
    ...
    def forward(self, positive_edges, negative_edges, target):
        ...
        # print("positive_edges:", positive_edges.shape)
        # print("negative_edges:", negative_edges.shape)
        # exit(0)
...
class SignedGCNTrainer(object):
    ...
    def setup_dataset(self):
        ...
        # edge_num = len(self.edges["positive_edges"])
        # for i in range(edge_num):
        #
        #     edge = self.edges["positive_edges"][i]
        ...
        # sub_graph_size = int(edge_num * self.args.test_size)
        #
        # selected_edge_index, no_selected_edge_index = get_bfs_sub_graph(self.edges["positive_edges"], node_num,
        #                                                                 node_to_edge_index, sub_graph_size)
        # # print(len(self.edges["positive_edges"]))
        # # print(len(selected_edge_index))
        # # print(len(no_selected_edge_index))
        # self.positive_edges = []
        # self.test_positive_edges = []
        # for i, x in enumerate(self.edges["positive_edges"]):
        #     if i in selected_edge_index:
        #         self.test_positive_edges.append(x)
        #     else:
        #         self.positive_edges.append(x)

        # node_to_edge_index_n = {}
        # # edge_num = int(self.edge_num // 2)
        # edge_num_n = len(self.edges["negative_edges"])
        #
        # for i in range(edge_num_n):
        #
        #     edge_n = self.edges["negative_edges"][i]
        #     if edge_n[0] not in node_to_edge_index_n.keys():
        #         node_to_edge_index_n[edge_n[0]] = []
        #     node_to_edge_index_n[edge_n[0]].append(i)
        #
        #     if edge_n[1] not in node_to_edge_index_n.keys():
        #         node_to_edge_index_n[edge_n[1]] = []
        #     node_to_edge_index_n[edge_n[1]].append(i)

        # node_num_n = len(node_to_edge_index_n)
        # sub_graph_size_n = int(edge_num_n * self.args.test_size)
        #
        # selected_edge_index_n, no_selected_edge_index_n = get_bfs_sub_graph(self.edges["negative_edges"], node_num_n,
        #                                                                     node_to_edge_index_n, sub_graph_size_n)
        # # print(len(self.edges["negative_edges"]))
        # # print(len(selected_edge_index_n))
        # # print(len(no_selected_edge_index_n))
        # self.negative_edges = []
        # self.test_negative_edges = []
        # for i, x in enumerate(self.edges["negative_edges"]):
        #     if i in selected_edge_index_n:
        #         self.test_negative_edges.append(x)
        #     else:
        #         self.negative_edges.append(x)
        # self.positive_edges, self.test_positive_edges = self.edges["positive_edges"][selected_edge_index], \
        #                                                 self.edges["positive_edges"][no_selected_edge_index]
        self.negative_edges, self.test_negative_edges = train_test_split(self.edges["negative_edges"],
                                                                         test_size=self.args.test_size)
        # print(self.positive_edges)
        # print(self.negative_edges)
        self.ecount = len(self.positive_edges + self.negative_edges)
        ...
        self.X = np.load('./src/features/Epinions_all1.npy')
        ...
        # print(self.positive_edges.shape[1])

        self.negative_edges = torch.from_numpy(np.array(self.negative_edges,
                                                        dtype=np.int64).T).type(torch.long).to(self.device)
        # print(self.negative_edges.shape[1])
    ...
    def score_model(self, epoch):
        ...
        targets = [1] * len(self.test_positive_edges) + [0] * len(self.test_negative_edges)
        ...
        # print(test_positive_z.shape)
        # print(test_negative_z.shape)
        # print(predictions.shape)
        # print(targets)
        if True:
            with open('./Epinions_target.pkl', 'wb') as f:
                pickle.dump(targets, f)

        if True:
            with open('./Epinions_prediction1.pkl', 'wb') as f:
                pickle.dump(predictions, f)
        # if True:
        #     with open('./Alpha_score.pkl', 'wb') as f:
        #         pickle.dump(predictions, f)
        # label = open('/home/home/Desktop/Code/SGCN/Alpha_target.pkl', 'rb')
        # label = np.array(list(enumerate(pickle.load(label))))
        # label = label.T
        # ind = np.argsort(label[0])
        # label = label[:, ind]
        # r1 = open('/home/home/Desktop/Code/SGCN/Alpha_prediction1.pkl', 'rb')
        # r1 = list(enumerate(pickle.load(r1)))
        # r1 = sorted(r1, key=lambda x: x[0])
        # r2 = open('/home/home/Desktop/Code/SGCN/Alpha_prediction2.pkl', 'rb')
        ...
        #     _, r22 = r2[i]
        #     r = r11 * (1 - 0.5) + r22 * 0.5
        # auc2, f12, pos_ratio2 = calculate_auc(targets, predictions, self.edges)
        # print(auc2, f12, pos_ratio2)
    ...
    def save_model(self):
        ...
        self.regression_params = pd.DataFrame(np.concatenate((self.regression_weights, self.regression_bias), axis=1),
                                              columns=regression_header)
